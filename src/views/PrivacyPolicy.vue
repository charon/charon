...
onBeforeMount(async () => {
  ...
  try {
    ...
    privacyPolicy.value = html
  } catch (err) {
    if (abortController.signal.aborted) {
      return
    }
    // TODO: Show some error to the user.
    console.error("PrivacyPolicy.onBeforeMount", err)
  } finally {
    progress.value -= 1
  }
})
...
<template>
  ...
  <div class="mt-12 flex w-full flex-col items-center border-t border-transparent sm:mt-[4.5rem]">
    <div class="m-1 grid auto-rows-auto grid-cols-[minmax(0,65ch)] gap-1 sm:m-4 sm:gap-4">
      <div class="prose w-full rounded-sm border border-gray-200 bg-white p-4 shadow-sm" v-html="privacyPolicy" />
    </div>
  </div>
  <Teleport to="footer">
    <Footer />
