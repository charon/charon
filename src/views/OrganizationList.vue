...
<template>
  ...
  <div class="mt-12 flex w-full flex-col items-center border-t border-transparent sm:mt-[4.5rem]">
    <div class="m-1 grid auto-rows-auto grid-cols-[minmax(0,_65ch)] gap-1 sm:m-4 sm:gap-4">
      ...
      <template v-else>
        <div v-if="!organizations.length" class="w-full rounded-xs border border-gray-200 bg-white p-4 italic shadow-sm">
          {{ isSignedIn() ? t("views.OrganizationList.noOrganizationsCreate") : t("views.OrganizationList.noOrganizationsSignIn") }}
        </div>
        <div v-for="organization in organizations" :key="organization.id" class="w-full rounded-xs border border-gray-200 bg-white p-4 shadow-sm">
