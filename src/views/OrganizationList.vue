...
<template>
  ...
  <div class="mt-12 flex w-full flex-col items-center border-t border-transparent sm:mt-[4.5rem]">
    <div id="organizationlist-content-list" class="m-1 grid grid-cols-[minmax(0,65ch)] gap-1 sm:m-4 sm:gap-4">
      <div class="flex w-full flex-col gap-4 rounded-sm border border-gray-200 bg-white p-4 shadow-sm">
        <div class="flex flex-row items-center justify-between gap-4">
          ...
          <ButtonLink v-if="isSignedIn()" id="organizationlist-button-create" :to="{ name: 'OrganizationCreate' }" :progress="progress" primary>{{
      ...
      <template v-else>
        <div v-if="!organizations.length" class="w-full rounded-sm border border-gray-200 bg-white p-4 italic shadow-sm">{{
