...
<template>
  ...
  <div class="w-full flex flex-col items-center mt-12 sm:mt-[4.5rem] border-t border-transparent">
    <div class="grid auto-rows-auto grid-cols-[minmax(0,_65ch)] m-1 sm:m-4 gap-1 sm:gap-4">
      ...
      <template v-else>
        <div v-if="!users.length" class="w-full rounded-sm border border-gray-200 bg-white p-4 shadow-sm italic">{{ t("views.OrganizationUsers.noUsers") }}</div>
        <div v-for="user in users" :key="user.id" class="w-full rounded-sm border border-gray-200 bg-white p-4 shadow-sm">
