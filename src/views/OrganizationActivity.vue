...
<template>
  ...
  <div class="w-full flex flex-col items-center mt-12 sm:mt-[4.5rem] border-t border-transparent">
    <div class="grid auto-rows-auto grid-cols-[minmax(0,_65ch)] m-1 sm:m-4 gap-1 sm:gap-4">
      ...
      <template v-else>
        <div v-if="!activities.length" class="w-full rounded-sm border bg-white p-4 shadow-sm italic">
          {{ t("views.OrganizationActivity.noActivities") }}
        </div>
        <div v-for="activity in activities" :key="activity.id" class="w-full rounded-sm border bg-white p-4 shadow-sm">
