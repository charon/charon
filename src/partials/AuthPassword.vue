  ...
  <div class="flex flex-col rounded border bg-white p-4 shadow w-full">
    <div class="flex flex-col">
      <label for="email-or-username" class="mb-1">{{
        isEmail(flow.getEmailOrUsername()) ? t("auth.password.emailAddressLabel") : t("auth.password.usernameLabel")
      }}</label>
    ...
    <div class="flex flex-col mt-4">
      <label for="current-password" class="mb-1">{{ t("auth.password.passwordLabel") }}</label>
      <!--
    ...
    <template v-if="passwordError">
      ...
      <div v-if="passwordError === 'wrongPassword'" class="mt-4">
        <i18n-t keypath="auth.password.troublePassword" scope="global">
          <template #link>
    ...
    <div class="mt-4 flex flex-row justify-between gap-4">
      ...
      -->
      <Button type="button" primary tabindex="3" :disabled="!!codeError" :progress="progress" @click.prevent="onCode">{{ t("auth.password.sendCodeButton") }}</Button>
    </div>
  </div>
</template>
