...
<template>
  <form class="flex flex-col" novalidate @submit.prevent="onSubmit">
    <label for="username" class="mb-1"
      >{{ t("labels.username") }}<span class="text-neutral-500 italic text-sm"> {{ t("labels.optional") }}</span></label
    >
    ...
    <label for="email" class="mb-1 mt-4"
      >{{ t("labels.email") }}<span class="text-neutral-500 italic text-sm"> {{ t("labels.optional") }}</span></label
    >
    <InputText id="email" v-model="email" class="flex-grow flex-auto min-w-0" :progress="progress" />
    <label for="givenName" class="mb-1 mt-4"
      >{{ t("labels.givenName") }}<span class="text-neutral-500 italic text-sm"> {{ t("labels.optional") }}</span></label
    >
    ...
    <label for="fullName" class="mb-1 mt-4"
      >{{ t("labels.fullName") }}<span class="text-neutral-500 italic text-sm"> {{ t("labels.optional") }}</span></label
    >
    ...
    <label for="pictureUrl" class="mb-1 mt-4"
      >{{ t("labels.pictureUrl") }}<span class="text-neutral-500 italic text-sm"> {{ t("labels.optional") }}</span></label
    >
    ...
    <label for="description" class="mb-1 mt-4"
      >{{ t("labels.description") }}<span class="text-neutral-500 italic text-sm"> {{ t("labels.optional") }}</span></label
    >
